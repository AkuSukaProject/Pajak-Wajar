...
- 183 tes rutin pada 12 berkas; satu tes layanan nyata hanya berjalan lewat `npm run test:ocr` dengan API key.
...
- Kamus istilah: `src/lib/istilah.ts` memuat 21 istilah dengan penjelasan bahasa sehari-hari, ditampilkan lewat komponen `Istilah` yang dapat ditekan (tutup dengan Escape atau klik di luar). Isinya bahan bacaan, bukan aturan; `tests/istilah.test.ts` menolak sitasi pasal di dalamnya agar dasar hukum tetap datang dari mesin aturan.
